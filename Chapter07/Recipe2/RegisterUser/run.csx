                        ...
                        )
{
  ...
  string emailContent = EMailFormatter.FrameBodyContent(firstname,lastname,email,profilePicUrl);
  string strAttachmentName = firstname + "_" + lastname + ".log";

  message = Helper.SendMail(strSubject,emailContent,strFromEmailAddress,email,strAttachmentName);

  using (var emailLogBloboutput = binder.Bind<TextWriter>(new BlobAttribute($"userregistrationemaillogs/{objInsertedUser.RowKey}.log")))
  {
